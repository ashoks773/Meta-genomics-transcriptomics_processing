#-- Filter7 Ecoli Removed
library(monocle3)
uniref_counts <- read.csv(file="Ecoli_removed/Bowtie2_map_uniRefGeneCounts_Filter7.csv.eColiRemoved.csv", sep = ",", row.names = 1) 
...
UniRef_counts_Dataset_filter7EcoliRemoved <- new_cell_data_set(as.matrix(uniref_counts_ordered),
                                                  cell_metadata = meta_filtered,
                                                  gene_metadata = UniRef_names)

UniRef_counts_Dataset_filter7EcoliRemoved <- preprocess_cds(UniRef_counts_Dataset_filter7EcoliRemoved, num_dim = 50)
#UniRef_counts_Dataset_filter7EcoliRemoved <- align_cds(UniRef_counts_Dataset_filter7EcoliRemoved, alignment_group = "Participant.ID") # To remove batch effect

## Step 3: Reduce the dimensions using UMAP
UniRef_counts_Dataset_filter7EcoliRemoved <- reduce_dimension(UniRef_counts_Dataset_filter7EcoliRemoved)

plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, label_groups_by_cluster=FALSE,  color_cells_by = "Participant.ID")
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, label_groups_by_cluster=FALSE,  color_cells_by = "diagnosis")

#Cluster the cells
UniRef_counts_Dataset_filter7EcoliRemoved <- cluster_cells(UniRef_counts_Dataset_filter7EcoliRemoved)
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, color_cells_by = "partition")

## Step 5: Learn a graph
UniRef_counts_Dataset_filter7EcoliRemoved <- learn_graph(UniRef_counts_Dataset_filter7EcoliRemoved)

## Step 6: Order cells
# UniRef_counts_Dataset_filter7EcoliRemoved <- order_cells(UniRef_counts_Dataset_filter7EcoliRemoved) Don't run this command if you are Running this on Cluster

# a helper function to identify the root principal points:
get_earliest_principal_node <- function(UniRef_counts_Dataset_filter7EcoliRemoved, time_bin="CD"){
  cell_ids <- which(colData(UniRef_counts_Dataset_filter7EcoliRemoved)[, "diagnosis"] == time_bin)
  
  closest_vertex <-
    UniRef_counts_Dataset_filter7EcoliRemoved@principal_graph_aux[["UMAP"]]$pr_graph_cell_proj_closest_vertex
  closest_vertex <- as.matrix(closest_vertex[colnames(UniRef_counts_Dataset_filter7EcoliRemoved), ])
  root_pr_nodes <-
    igraph::V(principal_graph(UniRef_counts_Dataset_filter7EcoliRemoved)[["UMAP"]])$name[as.numeric(names
                                                                                       (which.max(table(closest_vertex[cell_ids,]))))]
  
  root_pr_nodes
}
UniRef_counts_Dataset_filter7EcoliRemoved <- order_cells(UniRef_counts_Dataset_filter7EcoliRemoved, root_pr_nodes=get_earliest_principal_node(UniRef_counts_Dataset_filter7EcoliRemoved))

jpeg("Check_Filtering/Trajectory_diagnosis_Filter7EcoliRemoved.jpg", height = 5, width = 5, units = 'in', res = 600)
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, color_cells_by = "diagnosis")
dev.off ()
jpeg("Check_Filtering/Trajectory_pseudotime_Filter7EcoliRemoved.jpg", height = 5, width = 5, units = 'in', res = 600)
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, color_cells_by = "pseudotime")
dev.off ()
jpeg("Check_Filtering/Trajectory_PID_Filter7EcoliRemoved.jpg", height = 7, width = 7, units = 'in', res = 600)
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, color_cells_by = "Participant.ID")
dev.off ()
plot_cells(UniRef_counts_Dataset_filter7EcoliRemoved, color_cells_by = "Participant.ID")
